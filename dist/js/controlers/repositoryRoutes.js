"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.deleteRepository = exports.updateRepository = exports.getRepository = exports.addRepository = exports.getRepositories = void 0;
var getRepositories = function (req, res) {
    res.json('getRepositories route not implamented yet');
};
exports.getRepositories = getRepositories;
var addRepository = function (req, res) {
    res.json('addRepository route not implamented yet');
};
exports.addRepository = addRepository;
var getRepository = function (req, res) {
    res.json('getRepository route not implamented yet');
};
exports.getRepository = getRepository;
var updateRepository = function (req, res) {
    res.json('updateRepository route not implamented yet');
};
exports.updateRepository = updateRepository;
var deleteRepository = function (req, res) {
    res.json('deleteRepository route not implamented yet');
};
exports.deleteRepository = deleteRepository;
//# sourceMappingURL=repositoryRoutes.js.map